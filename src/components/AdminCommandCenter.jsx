...
export default function AdminCommandCenter({ onLogout }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#353534] pb-6">
        <div>
          ...
          <h1 className="text-3xl font-headline font-bold text-white flex items-center space-x-3">
            ...
            <span className="px-3 py-1 bg-[#97d700]/20 text-[#97d700] border border-[#97d700]/40 rounded-full text-xs font-mono font-bold">
              ADMIN AUTHENTICATED
            </span>
          </h1>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="px-4 py-2 bg-[#1c1b1b] hover:bg-[#252424] text-white font-mono text-xs rounded-xl border border-white/10 transition flex items-center space-x-1.5"
          >
      ...
      {selectedCase && (
        <div className="bg-[#1c1b1b] p-6 rounded-3xl border-2.5 border-[#00E5FF] shadow-neo space-y-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-4">
            <div>
              <div className="flex items-center space-x-2">
                <ShieldCheck className="w-5 h-5 text-[#97d700]" />
                <h2 className="font-headline font-bold text-white text-lg">
                  3rd Level Admin Verdict Confirmation
                </h2>
              </div>
              <p className="text-xs text-gray-300 font-mono mt-1">
                Active Case: <strong className="text-[#00E5FF]">{selectedCase.id}</strong> | Document: <strong className="text-white">{selectedCase.documentName}</strong> | Submitted by: <strong className="text-[#97d700]">{selectedCase.userName} ({selectedCase.userEmail})</strong>
              </p>
            </div>

            {/* Action Feedback */}
            {actionDoneMsg ? (
              <div className="px-5 py-2.5 bg-[#97d700]/20 text-[#97d700] border border-[#97d700] rounded-xl font-mono text-xs font-bold animate-pulse flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-red-600 text-white font-headline font-extrabold text-xs rounded-xl shadow hover:bg-red-500 active:scale-95 transition flex items-center space-x-1.5"
                >
                  <X className="w-4 h-4" />
          ...
          <div className="flex items-center space-x-3 text-xs font-mono">
            ...
            <input
              ...
              className="flex-1 bg-[#131313] border border-white/10 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-[#00E5FF]"
            />
          </div>
        </div>
      )}
      ...
      <div className="bg-[#1c1b1b] p-6 rounded-3xl border border-white/10 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center space-x-2">
            <Database className="w-5 h-5 text-[#97d700]" />
          ...
          <div className="flex items-center space-x-3">
            ...
            <button 
              onClick={reloadData}
              className="p-2 bg-[#131313] hover:bg-white/10 rounded-xl border border-white/10 text-[#00E5FF]"
              ...
              {filteredSubmissions.map((item) => (
                <tr 
                  ...
                  className={`cursor-pointer transition ${
                    selectedCase?.id === item.id ? 'bg-[#201f1f]' : 'hover:bg-white/5'
                  }`}
                >
                  <td className="py-4 px-3">
                    <div className="font-bold text-white">{item.id}</div>
                    <div className="text-[10px] text-[#97d700]">{item.userName} ({item.userEmail})</div>
                  </td>

                  <td className="py-4 px-3">
                    <div className="font-bold text-gray-200">{item.documentName}</div>
                    <div className="text-[10px] text-gray-400">{item.sector}</div>
                  </td>

                  <td className="py-4 px-3 font-bold text-sm">
                    <span className={item.riskScore > 80 ? 'text-red-400' : (item.riskScore > 15 ? 'text-yellow-400' : 'text-[#97d700]')}>
                      {item.riskScore}%
                    </span>
                  </td>

                  <td className="py-4 px-3">
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold border border-white/10 bg-[#131313] text-gray-300">
                      {item.verdict}
                    </span>
                  </td>
                  ...
                  <td className="py-4 px-3">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold border ${
                      item.adminConfirmed === 'AUTHENTIC'
                        ? 'bg-[#97d700]/20 text-[#97d700] border-[#97d700]/40'
                  ...
                  <td className="py-4 px-3 text-right">
                    <button
                      onClick={() => setSelectedCase(item)}
                      className="px-3 py-1.5 bg-[#131313] hover:bg-[#97d700] text-gray-300 hover:text-black font-bold rounded-lg border border-white/10 transition inline-flex items-center space-x-1"
                    >
      ...
      {selectedCase && (
        <div className="space-y-4 bg-[#1c1b1b] p-6 rounded-3xl border border-white/10">
          <h3 className="text-lg font-headline font-bold text-white flex items-center space-x-2">
            <span className="material-symbols-outlined text-[#00E5FF]">biotech</span>
            <span>Document Inspection — {selectedCase.documentName}</span>
          </h3>
          
          <div className="flex flex-col md:flex-row gap-8 items-start">
            <div className="flex-1 w-full flex justify-center bg-black/40 p-4 rounded-xl border border-white/5">
              <img 
                src={selectedCase.preview} 
                alt="Document Preview" 
                className="max-h-[500px] object-contain rounded-lg shadow-neo"
              />
            </div>
            
            <div className="w-full md:w-1/3 space-y-6">
              <div className="bg-[#131313] p-5 rounded-2xl border border-white/10 space-y-2">
                <div className="text-xs font-mono text-gray-400">AI MODEL VERDICT</div>
                <div className="text-2xl font-headline font-bold text-white">{selectedCase.verdict}</div>
              </div>
              
              <div className="bg-[#131313] p-5 rounded-2xl border border-white/10 space-y-2">
                <div className="text-xs font-mono text-gray-400">AI RISK SCORE</div>
                <div className={`text-4xl font-headline font-extrabold ${selectedCase.riskScore > 80 ? 'text-red-400' : (selectedCase.riskScore > 15 ? 'text-yellow-400' : 'text-[#97d700]')}`}>

...
export default function VerificationPage({ setActiveTab }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 pb-16">
      ...
      <div className="bg-[#1c1b1b] p-6 rounded-2xl border-2.5 border-[#00E5FF]/40 shadow-neo space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-headline font-bold text-white mt-1">
              Multi-Layer Document Verification
            </h1>
          </div>

          {/* Quick Demo Fixture Buttons */}
          <div className="flex items-center space-x-2 bg-[#131313] p-1.5 rounded-xl border border-white/10 overflow-x-auto">
            <span className="text-xs font-mono text-gray-400 px-2 whitespace-nowrap">Load Demo Fixture:</span>
              ...
              <div
                ...
                className={`p-2.5 rounded-xl border transition ${
                  isFailed
                    ...
                    : (isCompleted
                      ? 'bg-emerald-950/40 border-[#97d700] text-[#97d700] font-bold'
                      : 'bg-[#131313] border-white/5 text-gray-500')
                }`}
              >
                <p className="font-bold text-[11px] truncate">{layer.name}</p>
                <p className="text-[9px] text-gray-400 font-mono truncate">{layer.file}</p>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-headline font-bold text-white flex items-center space-x-2">
                <span className="material-symbols-outlined text-[#97d700]">biotech</span>
              ...
              <button
                ...
                disabled={isScanning}
                className="flex items-center space-x-1.5 text-xs font-mono text-[#00E5FF] hover:underline disabled:opacity-50"
              >
          ...
          {/* UPLOAD DROPZONE */}
          <div className="bg-[#1c1b1b] p-6 rounded-2xl border-2 border-dashed border-[#00E5FF]/30 hover:border-[#97d700] transition text-center space-y-3 relative group">
            <input
              type="file"
              accept="image/*,.pdf"
            ...
            />
            <div className="w-12 h-12 rounded-xl bg-[#201f1f] text-[#97d700] mx-auto flex items-center justify-center group-hover:scale-110 transition">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className={`p-6 rounded-2xl border-2.5 transition-all ${
            pipelineResult?.verdict === 'FORGERY'
              ...
              : (pipelineResult?.verdict === 'SUSPICIOUS'
                ...
                : 'bg-emerald-950/40 border-[#97d700] shadow-xl shadow-[#97d700]/10')
          }`}>
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <span className="text-xs font-mono text-gray-300">MULTI-VECTOR RISK FUSION VERDICT</span>
            ...
            <div className="mt-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className={`px-4 py-1.5 rounded-full text-xs font-mono font-extrabold border ${
                  pipelineResult?.verdictBadge || 'bg-gray-800 text-gray-300'
                }`}>
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-[#131313] hover:bg-white/10 text-white font-mono text-xs rounded-xl border border-white/10 transition flex items-center space-x-1.5"
                >
              ...
              <div className="pt-3 border-t border-white/10 grid grid-cols-3 gap-2 text-[10px] font-mono text-center">
                <div className={`p-2 rounded-lg border ${pipelineResult?.verdict === 'AUTHENTIC' ? 'bg-[#97d700]/20 border-[#97d700] text-[#97d700] font-bold' : 'bg-[#131313] text-gray-500'}`}>
                  🟢 0 - 15%<br />AUTHENTIC
                </div>
                <div className={`p-2 rounded-lg border ${pipelineResult?.verdict === 'SUSPICIOUS' ? 'bg-[#FFAB00]/20 border-[#FFAB00] text-[#FFAB00] font-bold' : 'bg-[#131313] text-gray-500'}`}>
                  🟡 16 - 84%<br />SUSPICIOUS
                </div>
                <div className={`p-2 rounded-lg border ${pipelineResult?.verdict === 'FORGERY' ? 'bg-red-500/20 border-red-500 text-red-400 font-bold' : 'bg-[#131313] text-gray-500'}`}>
                  🔴 85 - 100%<br />FORGERY
                </div>
              </div>
            </div>
            ...
            {pipelineResult && (pipelineResult.verdict === 'SUSPICIOUS' || pipelineResult.verdict === 'FORGERY') && (
              <div className="mt-5 pt-4 border-t border-white/10 flex items-center justify-between">
                <span className="text-xs text-gray-300 font-mono">Dispatched to Verifier Worklist:</span>
                <button
          ...
          <div className="bg-[#1c1b1b] p-6 rounded-2xl border border-white/10 space-y-4">
            <h3 className="text-sm font-headline font-bold text-white flex items-center justify-between">
              <span className="flex items-center space-x-2">
                <Cpu className="w-4 h-4 text-[#00E5FF]" />
                <span>6-Layer Forensic Analysis Findings</span>
              </span>
              <span className="text-xs font-mono text-[#97d700]">PARALLEL EXECUTION</span>
            </h3>
                ...
                <div 
                  ...
                  className={`p-3 rounded-xl border text-xs space-y-1.5 ${
                    layer.status === 'FLAGGED'
                      ...
                      : 'bg-[#131313] border-white/5 text-gray-300'
                  }`}
                >
                  <div className="flex items-center justify-between font-mono">
                    <span className="font-bold text-white">{layer.name}</span>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                      layer.status === 'FLAGGED' ? 'bg-red-500/20 text-red-400' : 'bg-[#97d700]/20 text-[#97d700]'
                    }`}>
                      {layer.status}
                    </span>
                  </div>
                  ...
                  <p className="text-[11px] font-mono text-gray-300">
                    <strong className="text-[#00E5FF] font-mono">{layer.code}: </strong>
                    {layer.detail}
                  </p>

                  <div className="flex items-center space-x-1 text-[10px] text-gray-400 font-mono pt-1 border-t border-white/5">
                    <HelpCircle className="w-3 h-3 text-[#97d700]" />
